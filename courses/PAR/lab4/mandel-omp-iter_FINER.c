	      ...
	      double CmaxI, double scale_real, double scale_imag, int maxiter)
{
  ...
  for (int y = 0; y < ROWS; y += TILE)
    ...
    {	
		int equal0 = 1;
		#pragma omp taskloop firstprivate(equal0)
		for (int px = x; px < x + TILE; px++)
		{
			...
			equal0 = equal0 && (M[y][x] == M[y][px]);
			equal0 = equal0 && (M[y][x] == M[y + TILE - 1][px]);
		}

		int equal1 = 1;
		#pragma omp taskloop firstprivate(equal1)
